                  ...
                  <div class="card">
                      ...
                      <div class="card-body">
                          <table id="example1" class="table table-bordered table-striped">
                              ...
                              <tbody>
                                @foreach($acedamic_year as $item)
                                  <tr>
                                      <td>{{$item->id}}</td>
                                      <td>{{$item->name_year}}
                                      </td>
                                      <td>{{$item->created_at}}</td>
                                      <td> {{$item->status == 1 ? 'Published' : 'Hidden'}}</td>
                                      <td><a href="{{route('admin.acedamicYear_edith', $item->id)}}" class = "btn btn-primary">Edith</a></td>
                                      <td><a href="{{route('admin.acedamicYear_delete', $item->id)}}" class="btn btn-danger" onclick = "confirmation(event)">Delete</a></td>
                                  </tr>
                                @endforeach
                              </tbody>
                              <tfoot>
